...
def scaling(histogram, frac, ndata):
    '''Scales the histogram to the data'''
    histogram.Scale(frac*ndata/histogram.Integral()) 

def componentFitter(preselEffDict,preselFracDict,eff4_5Dict,eventsDict,selection,lepton): 
...
#    s1 = s[0].split("_")
    ...
    path_name = "graphs/frac_%s_%s.pdf"%(selection,lepton)
    path_name2 = "graphs/frac_comp_Solver_%s_%s.pdf"%(selection,lepton)
    
#Opens ROOT file
    ROOTFile = r.TFile.Open("data/control_top_%s_ph_sn_jn_20.root"%(lepton))
    tf = ROOTFile.Get("Moments2Sum_triD_%s"%selection)
...
#Dictionaries of the components of the background and tt 5-jets
    fJetFracs = fiveJetEfficiencyFilter(preselEffDict,preselFracDict,eff4_5Dict,eventsDict)
    ttDict = dict(zip(fJetFracs.ttComps,fJetFracs.tt_5jet))
    bgDict = dict(zip(fJetFracs.bgComps,fJetFracs.bg_5jet))
    
    fracQuark = ttDict["ttag"]+ttDict["ttqg"]
    fracGluon = ttDict["ttgg"]+ttDict["ttqq"]
    
#Total number of events
    
    data = hists["data"].ProjectionX()

#Creates histogram and fills them from file, efficiencies from file
    bg = r.TH1F("bg","bg",50,0,0.11)
    bg.Add(hists["wj"].ProjectionX(),hists["st"].ProjectionX(),bgDict["wj"],bgDict["st"])
    ttg = r.TH1F("ttg","ttg",50,0,0.11)
    ttg.Add(hists["ttgg"].ProjectionX(),hists["ttqq"].ProjectionX(),ttDict["ttgg"],ttDict["ttqq"])
    ttq = r.TH1F("ttq","ttq",50,0,0.11) 
    ttq.Add(hists["ttqg"].ProjectionX(),hists["ttag"].ProjectionX(),ttDict["ttqg"],ttDict["ttag"])
    bg1 = r.TH1F("bg1","bg1",50,0,0.11)
    bg1.Add(hists["wj"].ProjectionX(),hists["st"].ProjectionX(),bgDict["wj"],bgDict["st"])
    ttg1 = r.TH1F("ttg1","ttg1",50,0,0.11)
    ttg1.Add(hists["ttgg"].ProjectionX(),hists["ttqq"].ProjectionX(),ttDict["ttgg"],ttDict["ttqq"])
    ttq1 = r.TH1F("ttq1","ttq1",50,0,0.11)
    ttq1.Add(hists["ttqg"].ProjectionX(),hists["ttag"].ProjectionX(),ttDict["ttqg"],ttDict["ttag"]) 
    data = r.TH1F("data","data",50,0,0.11)
    data.Add(hists["data"].ProjectionX(),1)
...
#Scales the histograms
    ...
    sm2 = r.TH1F("%s %s TFracFit"%(selection,lepton),"%s %s TFracFit"%(selection,lepton),50,0,0.11)     #histogram sm2
...
#Outputs fractions to screen
    print "%s Selection, %s Channel"%(selection,lepton)
            
    print "fttg from Lhood: ", fttg, "+/-", dfttg
    print "fttq from Lhood: ", fttq, "+/-", dfttq
    print "Fraction Sum: ", sum([fttg,fttq,fbg])
    print "Calculated Fraction: ", fttq/sum([fttq,fttg]),"\n"
    '''        
################################################################################################################
    '''Component Solver'''
...
#Impliments the Component Solver
    ...
    print "Calculated Fraction: ", compSolver.fractions[1]/sum(compSolver.fractions),"\n"
    print "Expected Fraction: ",fracQuark/(fracGluon+fracQuark),"\n"
    print "Second: ",(ttDict["ttag"]+ttDict["ttqg"])/(ttDict["ttgg"]+ttDict["ttqg"]+ttDict["ttag"]+ttDict["ttqq"])
            
#Prints histograms to file                                           
#    c0.Print(path_name)
            
#Scales from the results of the Component Solver
    scaling(bg2,fJetFracs.backgroundFrac,ndata)
    scaling(ttq2,compSolver.fractions[1],ndata)
    scaling(ttg2,compSolver.fractions[0],ndata)
    ...
    sm4 = r.TH1F("%s %s compSolver"%(selection,lepton),"%s %s compSolver"%(selection,lepton),50,0,0.11)
...
#Opens a file for storing fraction values
    f = open("frac_%s_%s.txt"%(selection,lepton),"a")
    '''
